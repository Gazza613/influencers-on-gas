...
export async function recordLogin(email, ip) {
  const r = redis()
  const key = `login_history:${normEmail(email)}`
  try {
    await r.lpush(key, { ts: Date.now(), ip: ip || 'unknown' })
    await r.ltrim(key, 0, 49) // keep the 50 most recent
  } catch {}
}
export async function getLoginHistory(email) {
  try {
    const items = await redis().lrange(`login_history:${normEmail(email)}`, 0, 49)
    return (items || []).map(x => (typeof x === 'string' ? (() => { try { return JSON.parse(x) } catch { return null } })() : x)).filter(Boolean)
  } catch { return [] }
}

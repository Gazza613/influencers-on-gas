...
export async function notifyRenderDone(opts: { name: string; kind: "final-render" | "cut-ready"; url?: string | null }): Promise<{ sent: boolean }> {
  ...
  try {
    await sendEmail({ to: recipient(), subject, html: shell(heading, body, href, label) });
    return { sent: true };
  } catch {

...
export const SCENE_PEOPLE =
  "background people are present, SIZED TO THE SETTING - a FEW (roughly two to five) for a quiet place like a cafe, home or office, or a FULLER natural crowd for a busy public place (a stadium, event, market, festival or racecourse grandstand) - placed naturally the way real people actually occupy that space (seated at different tables, queuing loosely, walking past, filling grandstand tiers) and NEVER arranged in a uniform row or tidy line; " +
...
export function buildShotPrompt(o: {
  ...
  hasPeople: boolean; worldAnchored: boolean; lockedOutfit?: string; grade?: string; liveBg?: boolean;
}): string {

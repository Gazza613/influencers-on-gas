...
export function buildShotPrompt(o: {
  ...
  return [
    ...
    o.role === "a-roll"
      ? "COMPOSITION: place her eyes on the upper-third line with natural headroom (not too much dead space above); camera at eye level or just 10-15 degrees above for a flattering angle, never looking down on her. BACKGROUND: STRONGLY out of focus behind her (heavy, creamy bokeh, shallow depth of field) so NO object back there is sharp, distinct, identifiable or moving - the setting reads as a soft wash of colour and light, never a depicted scene, a crowd, vehicles, or racing horses (this shot is animated by a talking-photo engine that moves only her, so anything distinct behind her would FREEZE and look broken)."

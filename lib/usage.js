...
//
// Billing is centralized on ONE Higgsfield Ultra account ($99/mo, 3,000
// credits/mo, billed annually). On that plan almost every model is UNLIMITED —
// zero marginal cost. The ONLY variable cost is credit-consuming models (e.g.
// Veo 3.1) drawing down the 3,000 monthly credits. So "spend" = the fixed $99
// plan + any credit overage, NOT a charge per generation.
...
// ── Plan (Gary's account: Higgsfield Ultra, billed annually) ─────────────────
export const PLAN = { name: 'Ultra', monthlyUsd: 99, monthlyCredits: 3000 }
export const CREDIT_USD = PLAN.monthlyUsd / PLAN.monthlyCredits // ≈ $0.033/credit

// ── Model catalog ────────────────────────────────────────────────────────────
// unlimited:true ⇒ included on the Ultra plan, $0 marginal. `credits` is the
// per-generation credit draw for credit-consuming models (the only real cost).
export const MODELS = {
  // video
  seedance_2_0:      { kind: 'video', label: 'Seedance 2.0',      unlimited: true,  credits: 0 },
  seedance_2_0_fast: { kind: 'video', label: 'Seedance 2.0 Fast', unlimited: true,  credits: 0 },
  kling3_0:          { kind: 'video', label: 'Kling 3.0',         unlimited: true,  credits: 0 },
  kling2_6:          { kind: 'video', label: 'Kling 2.6',         unlimited: false, credits: 7 },  // not in Ultra unlimited list
  veo3_1:            { kind: 'video', label: 'Veo 3.1',           unlimited: false, credits: 58 }, // ~8s clip
  // image (all unlimited on Ultra)
...
export function videoCredits(model, duration = 5) {
  ...
  switch (model) {
    ...
    case 'kling2_6': return d >= 8 ? 14 : 7
    default:         return Math.round(d * ((info.credits || 15) / 5))
  }
}
...
export async function getCostsReport(emails, month) {
  ...
  const models = Object.entries(perModel).map(([model, count]) => {
    const info = modelInfo(model)
    const credits = info.unlimited ? 0 : count * (info.credits || 15)

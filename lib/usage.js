...
// ── Billing cycle (11th → 10th) ──────────────────────────────────────────────
// Reporting follows the Higgsfield credit-reset cycle, not the calendar month.
// A cycle is [most-recent 11th, next 11th); its key is the start date (YYYY-MM-DD).
export function cyclePeriod(date = new Date()) {
  const d = date instanceof Date ? date : new Date(date)
  let sy = d.getUTCFullYear(), sm = d.getUTCMonth()
  if (d.getUTCDate() < RESET_DAY) { sm -= 1; if (sm < 0) { sm = 11; sy -= 1 } }
  const start = new Date(Date.UTC(sy, sm, RESET_DAY))
  const end = new Date(Date.UTC(sy, sm + 1, RESET_DAY)) // Date.UTC normalizes overflow
  const key = start.toISOString().slice(0, 10)
  const fmt = (dt) => dt.toLocaleString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' })
  const label = `${fmt(start)} – ${fmt(new Date(end.getTime() - 86400000))}`
  return { key, label, start, end }
}

// Recent cycles (most recent first) for the dashboard selector.
export function recentCycles(count = 6) {
  const out = []
  const now = new Date()
  for (let i = 0; i < count; i++) {
    const c = cyclePeriod(new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - i, RESET_DAY)))

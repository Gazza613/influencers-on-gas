...
ROLES — the influencer SPEAKS in BOTH; the difference is framing, so the film feels like one continuous performance, not a talking head plus silent scenery. Classify every scene as ONLY 'a-roll' or 'b-roll' (never 'graphic'): 'a-roll' = HEAD-ON to camera, tighter framing, direct address (she looks into the lens and talks); 'b-roll' = the SAME influencer talking IN-SITU — a medium-to-wider shot of her in the location DOING something real (walking through the space, sitting, using/showing the product, turning to camera) while she speaks, glancing to camera and around the scene naturally rather than locked head-on. EVERY a-roll and most b-roll scenes have her speaking (a vo_line). Only an occasional pure-scenery cutaway (no influencer) may have an empty vo_line. Use a-roll for the direct beats (hook, CTA) and b-roll for the in-situ/demo beats (solution, proof) — she talks throughout. Motion_prompts include believable body movement and that background people move.

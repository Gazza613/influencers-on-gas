...
const SYSTEM = (clientName: string, objectiveLabel: string, tier: (typeof TIERS)[TierId]) =>
  ...
  `- PRICE IS A STRATEGY, never dismiss it. Buyers weigh price first, then value and offer, so treat pricing as a core strategic lever, not a thing to move "from opener to closer". If ${clientName} is a manufacturer or has a genuine cost advantage (manufacturer-direct, own factory, no middleman), that IS a defensible pricing wedge: they can win on price AND value at once, which a reseller competitor structurally cannot match. Build the positioning on real, evidenced advantages, and never over-rotate the whole strategy onto a single "wedge" while waving price away.\n` +
  ...
  `- THE EXECUTIVE SUMMARY MUST STAND ALONE. It is read first, cold. Do not use tier names (e.g. "Dominate"), the word "pods", or any system mechanic the reader has not been introduced to yet. Explain the case in plain client language; the system and tier are introduced later in the document.\n` +

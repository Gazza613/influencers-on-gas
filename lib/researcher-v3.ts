...
export async function prepareResearch(clientId: string, runId: string, version: number, today: string, opts: { userEmail?: string | null; notes?: string | null; focus?: string | null; plan?: Record<string, unknown> | null } = {}): Promise<ResearchCtx> {
  ...
  const brief = `Today is ${today}. Collect a verified fact base on ${name} for a marketing research brief.${focusBlock}${planBlock}${socialBlock}${ownerBlock}${knownList}${notesBlock}\n\n` +
    ...
    `- COMPETITORS (sections=competitor_set and competitor): NON-NEGOTIABLE - map ${name}'s WHOLE competitive field, not a token handful. Name every genuine rival (direct like-for-like AND adjacent players competing for the same spend), tier them, and profile the material ones (what they sell, positioning, price posture, manufacturer-vs-reseller, any guarantee/finance, footprint, channels). If a rival matches ${name}'s supposed differentiator, say so. A dedicated competitor pass runs after this one and goes deeper, but surface the real, comprehensive rival set here.\n` +
...
export async function researchCompetitors(ctx: ResearchCtx, rawClaimsIn: RawClaim[], competitorsIn: { name?: string; website?: string }[], emit: (e: CollectEvent) => void): Promise<{ rawClaims: RawClaim[]; competitors: { name?: string; website?: string }[] }> {
  ...
  const competitorBrief =
    ...
    `- Identify EVERY genuine rival, not a token handful. A real category has MANY: cover BOTH the direct like-for-like specialists AND the adjacent players who take the same spend (for a bed retailer that is dedicated bed/mattress chains AND the furniture retailers that also sell beds, AND the manufacturer-brands). Name the real, specific companies.\n` +

...
// it errors: Fable's web_search server-tool support is NOT documented, and the whole gather depends on web_search,
...
- COVER THE OUTSIDE, NOT JUST THE CLIENT. A strategist needs the OUTSIDE-IN picture as much as the client's own facts, and this is where research most often falls short. Give real, sourced depth to: the MARKET and category (size, growth rate, seasonality, demand and search-interest direction, consumer shifts, each with a real figure and a dated source, never a vague "the market is growing"); the COMPETITOR field (the whole set, the material rivals profiled and compared on price, model and offer); CUSTOMER VOICE (review platforms with volume, average rating and the recurring themes in customers' own words, HelloPeter and Google Reviews for South Africa); and the client's observable SCALE (follower counts, engagement, store/branch footprint). Thin market data is a common failure that leaves the downstream strategy data-light: dig until you have real, dated, sourced numbers, or record a specific gap. Never pad with a vague generality.
...
- MAP THE WHOLE COMPETITIVE FIELD, do not stop at a token handful. A real category has MANY rivals: identify them ALL - the direct like-for-like players AND the adjacent ones who compete for the same spend - and tier them (direct vs adjacent). Never leave the competitor set empty. If none have been supplied, find them yourself from the client's category, market and geography (default South Africa unless the ground truth says otherwise): real, specific, named companies. Profile the material players in depth and at least name and position the rest. For each material rival, file a competitor_set profile: what they sell and their range, their stated positioning/USP and tone, price posture and how they compete on price, whether they are a manufacturer or reseller, any guarantee/returns and finance/instalment offer, store/branch or scale footprint, official website and social handles, and one line on how they stack up against the client. If a rival MATCHES the client's supposed differentiator (a similar guarantee, finance, or manufacturer-direct pricing), say so plainly - the strategist needs to know the differentiator is not unique.
...
async function gatherAndFile(client: Anthropic, ctx: ResearchCtx, passBrief: string, maxSearches: number, emit: (e: CollectEvent) => void, fileInstructionOverride?: string): Promise<FileOut> {
  ...
  // that shared prefix from cache at ~0.1x instead of full price - a big cut on the biggest repeated input. The
  // system is IDENTICAL between the two calls (the file-specific instructions moved to the file step's user turn)
  ...
  await recordTokens({ clientId: ctx.clientId, userEmail: ctx.userEmail, model: RESEARCH_MODEL, action: "deep-research", inputTokens: gu?.input_tokens || 0, outputTokens: gu?.output_tokens || 0, cacheReadTokens: gu?.cache_read_input_tokens || 0, cacheCreationTokens: gu?.cache_creation_input_tokens || 0, webSearches: gu?.server_tool_use?.web_search_requests ?? 0 }).catch(() => {});
  ...
  await recordTokens({ clientId: ctx.clientId, userEmail: ctx.userEmail, model: RESEARCH_MODEL, action: "research-file", inputTokens: fu?.input_tokens || 0, outputTokens: fu?.output_tokens || 0, cacheReadTokens: fu?.cache_read_input_tokens || 0, cacheCreationTokens: fu?.cache_creation_input_tokens || 0 }).catch(() => {});
...
async function fileFromSite(client: Anthropic, ctx: ResearchCtx): Promise<RawClaim[]> {
  ...
  const filed = await withAnthropicRetry(() => client.messages.stream({
    model: RESEARCH_MODEL, max_tokens: 20000,
...
export async function prepareResearch(clientId: string, runId: string, version: number, today: string, opts: { userEmail?: string | null; notes?: string | null; focus?: string | null; plan?: Record<string, unknown> | null } = {}): Promise<ResearchCtx> {
  ...
  // IN SCOPE as context - not a competitor to wall off. It explains who backs the brand, their network, markets,
  // scale and credibility, all of which matter for deep research. The team supplies it as ground truth.
  const ownerContext = (socialRows[0]?.owner_context || "").trim();
  const ownerBlock = ownerContext
    ? `\n\nOWNER / PARENT CONTEXT - GROUND TRUTH from the GAS team, IN SCOPE for this research: ${ownerContext}\nResearch the owner/parent named here for CONTEXT and record the ownership relationship as a fact (section=foundations). USE the parent's markets, distribution, network, scale and credibility as context that backs ${name}. BUT attribute correctly: the parent's named executives, CEO, headcount and financials belong to the PARENT and must NEVER be presented as ${name}'s own. ${name} is the SUBJECT; the parent is its owner-context, clearly labelled as such.`
    : "";
  ...
  const brief = `Today is ${today}. Collect a verified fact base on ${name} for a marketing research brief.${focusBlock}${planBlock}${socialBlock}${ownerBlock}${knownList}${notesBlock}\n\n` +
    ...
    `- COMPETITORS (sections=competitor_set and competitor): NON-NEGOTIABLE - map ${name}'s WHOLE competitive field, not a token handful. Define the field by the CUSTOMER'S ALTERNATIVES (who this customer would choose INSTEAD), not by ${name}'s exact business model, and NEVER conclude there is no rival set. Name every genuine rival (direct like-for-like AND adjacent players competing for the same customer and spend), tier them, and profile the material ones (what they sell, positioning, price posture, manufacturer-vs-reseller, any guarantee/finance, footprint, channels). If a rival matches ${name}'s supposed differentiator, say so. A dedicated competitor pass runs after this one and goes deeper, but surface the real, comprehensive rival set here.\n` +
    `- AUDIENCE, IN DEPTH (section=audience): go well beyond who they SAY they serve. Build a real audience picture a strategist can position and create against: the distinct customer SEGMENTS, and for each capture DEMOGRAPHICS (age, gender, income band, location, life stage - where observable) AND PSYCHOGRAPHICS (values, attitudes, lifestyle, identity), their JOBS-TO-BE-DONE (what they hire this product or service to achieve), their PAINS and frustrations, and their BUYING TRIGGERS and occasions (what prompts a purchase, and any seasonality of need). Source it from the client's own audience-facing pages, from their reviews and social comments (how real customers describe themselves and their need), and from category audience research. Observed facts and patterns only, never an invented persona; where a segment is inferred from real evidence, say so.\n` +
    ...
    `- OBSERVABLE PERFORMANCE AND SCALE (sections=digital, customer_voice, snapshot): the PUBLIC proof points a strategist quotes, each observed and sourced, never estimated. Per social channel: the follower/subscriber COUNT and any visible engagement level. Per review platform (HelloPeter, Google, Facebook): the review VOLUME and the AVERAGE RATING, and whether it is trending up or down. Physical scale: the number of stores/branches/outlets and the geographic spread. If a number is not publicly visible, record its absence rather than guessing it.\n` +
    ...
    `- DEMAND AND "NOW" SIGNALS (section=market, and date-specific ones to activity): capture what the category is doing RIGHT NOW, distinct from the static market-size stats above. Current and rising consumer SEARCH and interest in the category (e.g. Google Trends direction, trending queries, topics gaining attention), the SEASONALITY of demand (when the category peaks and dips through the year), and live consumer-behaviour SHIFTS (how buying, channels or expectations are changing this year). This is the current-demand picture the strategy rides. Date and source every signal; describe the direction you can observe rather than inventing a precise figure.\n` +
    ...
    `- OWNER/PARENT, USE FOR CONTEXT BUT ATTRIBUTE CORRECTLY: if ${name} is a product or brand owned by, distributes for, or operates under a parent/holding company (see any OWNER/PARENT CONTEXT above), the parent is IN SCOPE as context, not a competitor to ignore. Research it, record the ownership relationship as a fact (section=foundations), and USE the parent's markets, distribution network, scale and credibility as context that backs ${name}. BUT keep the ATTRIBUTION strict: the parent's named executives, CEO, headcount and financials belong to the PARENT and must NEVER be presented as ${name}'s OWN. ${name}'s leadership means ${name}'s own people, never the parent's CEO.\n\n` +
    `${COMPETITOR_BRIEF}\n\n` +
...
export async function researchGatherPass1(ctx: ResearchCtx, emit: (e: CollectEvent) => void): Promise<{ rawClaims: RawClaim[]; competitors: { name?: string; website?: string }[]; vertical: string | null; identity: ResearchIdentity }> {
  ...
  const out = await gatherAndFile(client, ctx, ctx.brief + ctx.siteBlock, 32, emit);
...
export async function researchGapFill(ctx: ResearchCtx, rawClaimsIn: RawClaim[], competitorsIn: { name?: string; website?: string }[], emit: (e: CollectEvent) => void): Promise<{ rawClaims: RawClaim[]; competitors: { name?: string; website?: string }[] }> {
  ...
  const MANDATORY: { id: string; need: string; min: number; hint: string }[] = [
    ...
    { id: "audience", need: "audience DEPTH - segments with demographics + psychographics, jobs-to-be-done, pains, and buying triggers/occasions", min: 2, hint: `read ${ctx.name}'s audience-facing pages, their reviews and social comments (how customers describe themselves), and category audience research - capture segments, demographics, psychographics, jobs-to-be-done, pains and buying triggers` },
    ...
    { id: "market", need: "current market/category stats AND demand/NOW signals (search interest, seasonality, consumer shifts)", min: 2, hint: `search recent, dated category statistics for ${ctx.name}'s market AND current demand signals - search-interest direction, trending queries, seasonality and consumer-behaviour shifts` },
...
export async function researchCompetitors(ctx: ResearchCtx, rawClaimsIn: RawClaim[], competitorsIn: { name?: string; website?: string }[], emit: (e: CollectEvent) => void): Promise<{ rawClaims: RawClaim[]; competitors: { name?: string; website?: string }[] }> {
  ...
  const namedBlock = list.length
    ? `Start from these known rivals and PROFILE EACH in depth, then COMPLETE the picture by finding every other genuine competitor that is missing - do not stop at this list:\n${list.map((c) => `- ${c.name}${c.website ? ` (${c.website})` : ""}`).join("\n")}`
    : `No competitor has been named yet. Your job is to map the WHOLE competitive field for ${ctx.name} yourself (${ctx.name} is South African unless the ground truth says otherwise).`;
  ...
  const competitorBrief =
    ...
    `${namedBlock}\n\n` +
    `MAP THE WHOLE FIELD, COMPREHENSIVELY - this is the Researcher's job, do it to an expert standard:\n` +
    `- Identify EVERY genuine rival, not a token handful. Define the competitive frame by the CUSTOMER'S ALTERNATIVES - everyone this client's customer would realistically consider INSTEAD - NOT by the client's exact business model. Cover BOTH the direct like-for-like specialists AND the adjacent players who take the same customer and spend. Examples: for a bed retailer, dedicated bed/mattress chains AND furniture retailers that also sell beds AND manufacturer-brands; for a developer-landlord letting its own new-build stock, OTHER developer-landlords and buy-to-let / rental-investment companies (the likes of IGrow and Balwin) AND other new-build rental developments AND letting agents' comparable stock in the same areas. Name the real, specific companies.\n` +
    `- NEVER conclude there is "no rival set" or "no comprehensive rival set". Every business competes with someone for the same customer. If an unusual model makes the DIRECT like-for-like frame look empty, BROADEN to the adjacent set (who else the customer would choose) - do not stop and declare none. Returning no competitors is a research FAILURE, not a finding; if you cannot fully profile them, still NAME the real rival companies you can identify and tier them.\n` +
    `- TIER them: which are head-to-head direct rivals, and which are adjacent/substitute. Say which.\n` +
    `- PROFILE the material players in real depth (aim for the top 6 to 8 in full, and at least name and position the rest). For each: what they sell and their range, their stated positioning/USP and tone, price posture and how they compete on price, whether they are a manufacturer or a reseller (a real cost-structure difference), any guarantee/returns and finance/instalment offer, store/branch footprint and scale, official website and social handles.\n` +
    `- COMPARE where it matters: how does each stack up against ${ctx.name} on price/value, on manufacturer-vs-reseller cost advantage, and on any guarantee/finance? If a rival matches ${ctx.name}'s supposed differentiator (e.g. a similar guarantee or finance option), SAY SO plainly - that is exactly what the strategist needs.\n` +
    `- competitor (section): observable public activity per rival - current campaigns or promotions, notable moves in the last 90 days, product ranges pushed, and public review ratings (SA platforms included: HelloPeter, Google, Facebook).\n` +
  ...
  const fileInstruction =
    ...
    `PRICE FRESHNESS (30 DAYS): prices and ranges move too fast to trust when stale. Only state a rival's price as current, and only use it in a live head-to-head against ${ctx.name}, if it is sourced within about 30 days of today (${ctx.today}). A price older than 30 days, OR any price with no visible date, must NOT be presented as the current price or used as a live comparison - keep it only as context, tagged "(price dated {date}, not current, re-verify)" or "(price undated, re-verify before use)". Compare on price POSTURE (manufacturer-direct, discounter, premium) when a fresh figure is not available, rather than quoting a stale number as if live.\n` +
...
export async function researchReview(ctx: ResearchCtx, rawClaimsIn: RawClaim[], emit: (e: CollectEvent) => void): Promise<RawClaim[]> {
  ...
  if (rawClaims.length) {
    ...
    try {
      const qa = await withAnthropicRetry(() => client.messages.stream({
        model: RESEARCH_MODEL, max_tokens: 16000,
      ...
      await recordTokens({ clientId: ctx.clientId, userEmail: ctx.userEmail, model: RESEARCH_MODEL, action: "research-qa", inputTokens: qu?.input_tokens || 0, outputTokens: qu?.output_tokens || 0, cacheReadTokens: qu?.cache_read_input_tokens || 0, cacheCreationTokens: qu?.cache_creation_input_tokens || 0 }).catch(() => {});
  ...
  const GAP_CHECK: { id: string; label: string; min: number }[] = [
    ...
    { id: "audience", label: "audience depth - segments, demographics, psychographics, jobs-to-be-done and buying triggers", min: 2 },
    { id: "marketing", label: "their own current marketing and advertising", min: 1 },
    { id: "market", label: "current category stats and demand/NOW signals (search interest, seasonality, consumer shifts)", min: 2 },
    { id: "digital", label: "observable performance and scale - follower counts, engagement, review volume and average rating, store count", min: 1 },
    { id: "competitor_set", label: "a comprehensive rival set (direct and adjacent), the material ones profiled", min: 4 },

...
export const generateClips = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const renderOne = async (i: number, sc: Record<string, string>): Promise<ClipRow> => {
      ...
      const MOTION_SAFE = " Camera is SMOOTH and STABILISED (gimbal-steady), gentle and locked — absolutely no jittery or shaky handheld motion. Everyone and everything moves with real spatial awareness along physically believable paths: nobody walks into the pool, water, walls, furniture, plants or other people, and nobody clips through objects. All motion is grounded, natural and plausible. CRITICAL ON SPEED: everyone moves at EXACTLY NORMAL REAL-TIME speed, as if filmed live — never sped-up or hurried, but equally NEVER slow-motion, dreamy, floaty or drifting; real-world walking and gesture pace. Motion is smooth, fluid and continuous — NO stutter, judder, jitter, strobing, speed-ramping, fast-forward, slow-motion or time-lapse feel, and no jumpy or skipped frames. OBJECT PERMANENCE + PHYSICS (critical - this is where the video model fails most): every object stays SOLID, SINGLE, and obeys GRAVITY. Anything held (a phone, cup, mug, pen, book, product) stays FIRMLY GRIPPED IN THE SAME HAND for the ENTIRE clip - it NEVER flies out of the hand, is thrown, drops, floats, hovers, levitates in mid-air, drifts away or detaches from the hand. Anything resting on a surface (a mug or cup on the table, a notebook, a laptop) stays PUT on that surface and does not slide or lift off. NO object MORPHS, warps, melts, or TRANSFORMS INTO A DIFFERENT OBJECT - a coffee cup STAYS a coffee cup and never turns into a pen; a phone stays a phone; a mug stays a mug - each object keeps its exact shape, size, type and identity from the first frame to the last, and never multiplies, duplicates or swaps for something else. Hands and fingers stay anatomically correct and grip every object believably throughout. SKIN REALISM: preserve the REAL skin texture, visible pores and natural imperfections of the source image — do NOT smooth, wax, plastic-ify, airbrush, beautify or porcelain-glaze the skin; keep a believable matte, lived-in complexion. WORLD STABILITY (critical): the built environment is SOLID and RIGID — walls, floors, ceilings, doorways, windows, counters, tables, chairs, sofas, shelves, lamps, plants, umbrellas, parasols, signage, poles, railings, banners, bags and every fixed object or prop keep their EXACT shape, straight edges, size and position for the entire clip; they NEVER bend, warp, morph, ripple, melt, breathe, sway, stretch, wobble, slide or drift. ONLY living things (the people, their hair and clothing, water, leaves, steam) move; furniture, props and architecture stay perfectly still as in real footage. DURING ANY CAMERA MOVE (pan, dolly, push-in, drift): every fixed object stays ANCHORED to its exact spot in the world and shifts across the frame ONLY through natural camera parallax — it NEVER slides on its own, floats, follows or tracks the subject, warps, stretches or drifts; a standing umbrella, a sign or a pole stays planted exactly where it is as the camera passes it, exactly like real footage shot on a moving camera. MOTION RESTRAINT: keep overall motion SUBTLE and minimal — small, natural, real-life movements, not big sweeping action; when in doubt, less movement. SINGLE COHERENT ACTION (critical — stops the odd reversals): every person performs ONE natural, continuous action in ONE consistent direction for the whole clip. NOBODY walks toward camera and then abruptly turns and walks the other way, paces back and forth, reverses, backtracks, or shuffles/steps SIDEWAYS unnaturally — a person walking keeps walking the SAME way at a steady real-life pace, and a person standing stays put; no sudden about-turns. OCCLUSION CONTINUITY: when anyone passes BEHIND the subject or an object, they stay on their real path and RE-EMERGE correctly on the far side a moment later — they NEVER vanish, fail to reappear, duplicate into two people, teleport, pop in and out, loop or repeat. TREES + FOLIAGE STAY ROOTED: a tree, bush, palm or plant is FIXED to the ground — its trunk and its position never move, slide, drift, follow or travel with a camera pan; ONLY its leaves and thin branches may sway gently in a breeze. During any pan a tree shifts across frame ONLY through natural camera parallax, exactly like real footage, and stays planted in the world (it never detaches, floats or runs with the camera). HELD BAGS + CLUTCHES (critical): a handbag, clutch, purse or bag she holds moves WITH her hand and body as ONE connected unit - if she lifts, lowers, swings or moves her hands, the bag travels WITH them and stays firmly in her grip; it NEVER stays behind, hovers in its previous position, floats in mid-air where it was, or detaches while her hand moves away from it. BACKGROUND CROWD SPEED (critical - this fails constantly): background people and crowds move at NORMAL, REAL-TIME human pace, exactly the SAME natural speed as the foreground subject - they are NEVER sped-up, fast-forwarded, hurried, stuttering, strobing, jittery or time-lapsed; a distant crowd in a grandstand or on a street walks, stands and gestures at the same believable real-life speed as a person standing right next to the camera. PEOPLE ARE SOLID (critical - this fails badly): the subject and EVERY person are solid physical bodies that can NEVER overlap, merge or pass through one another. She NEVER walks INTO, THROUGH or over the top of another person, and never through a QUEUE or LINE of people - if anyone is in her path she stops, or steps and flows naturally AROUND them; she never clips, phases or ghosts through a person as if they were transparent. A queue or line of people (e.g. at a betting counter) stays intact and solid, and nobody walks through it. She also does not walk toward a group and then reverse and walk back through them - one natural, collision-free path.";
      ...
      const motion = (liveBg
        ...
        : `${base}. A natural, candid video SCENE: she is IN the environment doing something real (sitting, using or showing the product, a relaxed glance or small gesture) and is NOT looking at or talking to the camera — observed b-roll, not a piece to camera. NOBODY in the scene looks at, mouths words to, or addresses the camera; her mouth is NOT moving as if speaking to camera (her voice is a voiceover laid over the top).${soloBroll ? " The subject is ALONE in this scene, so their mouth stays RELAXED and CLOSED for the whole clip - they do NOT talk, mouth words, chew or move their lips as if speaking to themselves (a person talking to no one on screen looks broken); they are quietly, naturally focused on what they are doing - a calm, wordless moment." : " A named companion is present in this scene, so natural, relaxed conversation between them is fine - they may talk and react to each other (never to the camera), lips moving naturally as real people chatting."} The scene has GENTLE, restrained life: she moves naturally at a calm real-life pace with only SUBTLE ambient motion (a soft breeze, light shifting, any water or leaves). The CAMERA is almost locked off: at most ONE very slow, subtle, gentle straight PUSH-IN (a tiny forward move) OR a soft rack-focus onto her or the product, easing gently over the whole clip — and when in doubt, keep the frame STATIC. Do NOT pan, do NOT do a lateral dolly or sideways drift, do NOT arc, track, crane, swoop, whip-pan, snap-zoom or crash-zoom, and no hand-held shake. Lateral and panning moves are exactly where fixed things (trees, buildings, poles, signage, mountains) WARP, bend or hallucinate, because the camera has to reveal new parallax — so avoid them entirely; a tiny straight push-in keeps fixed structures the most stable. Throughout, EVERY fixed object — trees, buildings, walls, umbrellas, signs, poles, railings, furniture, mountains and skyline — stays perfectly RIGID and geometrically solid, holding its exact shape, size and straight edges, and shifts across frame ONLY through natural, minimal camera parallax; it NEVER bends, morphs, ripples, slides, stretches, grows extra branches or floors, duplicates or drifts. ${RIGID ? "This scene has prominent fixed structures (trees / buildings / a landmark) that distort easily on any move, so keep the camera ESSENTIALLY STILL — a barely-perceptible push-in at most, or a fully locked frame; favour locked over moving here." : "When unsure, a barely-there push-in or a fully locked frame."}`) + MOTION_SAFE + WATER;

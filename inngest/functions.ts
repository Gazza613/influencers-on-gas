  ...
  async ({ event, step }) => {
    ...
    const renderOne = async (i: number, sc: Record<string, string>): Promise<ClipRow> => {
      ...
      if (role === "a-roll" && audioUrl) {
        ...
        const prompt = `Natural, lifelike talking-to-camera delivery: ${base}. She FINISHES her sentence completely and then SETTLES calmly — mouth closing, a composed beat, holding her relaxed expression — never inhaling, never looking about to speak again, never cut mid-word. Her movement is entirely SUBJECT-driven, calm and human: gentle natural head movement, subtle shoulder and posture shifts, natural blinking and warm micro-expressions as she speaks. HANDS REST FOR MOST OF THE CLIP (in her lap, at her sides, or lightly clasped) and move only OCCASIONALLY — at most one or two SMALL, slow, LOW gestures kept CLOSE to her body and BELOW chest height; never raised high, never wide or sweeping, never repeated, choppy, waving, flailing or exaggerated. She is a calm, composed person who barely uses her hands — keep warm life in her FACE and gentle head movement, but the hands stay quiet and mostly still (do not eliminate gesture entirely, just keep it rare and understated). Real, like a person calmly talking to a friend. The camera holds a steady, gentle, essentially locked frame on her — no pan, tilt, zoom, push or crane — and she stays centred and fully in frame throughout. The BACKGROUND is a real room behind her with shallow depth of field, but it stays CALM and naturally STILL: do NOT animate, move, warp, duplicate or fast-forward any background people or objects — nobody in the background moves unnaturally. ONLY she moves, naturally and at real-life speed.`;

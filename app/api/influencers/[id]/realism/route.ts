...
// Run the hero through the realism pass (Magnific).
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!hero) return NextResponse.json({ error: "Build the identity first. There's no face to humanise yet." }, { status: 400 });

  try {
    await inngest.send({ name: "influencer/enhance.realism", data: { influencerId: id } });
  } catch {

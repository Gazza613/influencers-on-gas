...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await updateInfluencer(id, { persona: { ...persona, production: { ...production, assembly_status: "running", final_url: null } } });
  try {
    await inngest.send({ name: "influencer/assemble.video", data: { influencerId: id } });
  } catch {

    ...
    <div className="flex min-h-dvh flex-col">
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-6 py-10">
        ...
        <section className="mt-6 rounded-2xl border border-line bg-surface-1 p-5">
          <div className="flex flex-wrap items-center gap-3">
            <select value={clientId} onChange={(e) => setClientId(e.target.value)} className="rounded-lg border border-line bg-surface-2 px-3 py-2 text-[17px]">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <button onClick={plan} disabled={!!busy.plan || !!busy.all || brief.trim().length < 6}
              className="inline-flex items-center gap-2 rounded-lg border border-line px-4 py-2 text-base font-bold text-ink-dim hover:text-ink disabled:opacity-40">
              {busy.plan && <Spinner />}
        ...
        {err && <p className="mt-4 rounded-xl border border-red-500/40 bg-red-500/10 p-3 text-base font-semibold text-red-300">{err}</p>}
        ...
        {flags.length > 0 && (
          <details className="mt-4 rounded-xl border border-amber-500/30 bg-amber-500/[0.06] p-4">
            <summary className="cursor-pointer text-base font-bold text-amber-300">{flags.length} thing{flags.length === 1 ? "" : "s"} the Producer flagged for your team to check</summary>
              ...
              {Array.from({ length: sec.count }).map((_, slot) => {
                ...
                return (
                  <div key={slotKey} className="mt-4 rounded-2xl border border-line bg-surface-1 p-5">
                    {sec.count > 1 && <p className="mb-2 text-base font-bold text-ink-dim">Slider {slot + 1}</p>}
                    ...
                    {concept[slotKey] && (
                      <div className="mb-3 rounded-lg border border-[#818cf8]/30 bg-[#818cf8]/[0.06] px-3 py-2">
                        <p className="text-[14px] font-semibold uppercase tracking-wider text-[#a5b4fc]">Producer&apos;s direction</p>
                        <p className="mt-0.5 text-base leading-relaxed text-ink-dim">{concept[slotKey]}</p>
                      </div>
                    )}

                    {/* carousel of this section's references */}
                    <div className="flex gap-3 overflow-x-auto pb-2">
                      {options.length === 0 && <p className="text-base text-ink-faint">No {sec.title.toLowerCase()} designs uploaded at intake.</p>}
                      {options.map((r) => (
                    ...
                    <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                      ...
                      <div className="sm:col-span-2">
                        ...
                        {cardSel[slotKey] && <p className="mt-1 text-[14px] text-[#86efac]">✓ Real artwork - composited exactly, never redrawn.</p>}
                      </div>
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      <button onClick={() => generate(slotKey, sec.key)} disabled={!!busy[slotKey]}
                        className="inline-flex items-center gap-2 rounded-lg bg-accent px-5 py-2 text-base font-bold text-black disabled:opacity-40">
                      ...
                      {s && s.status !== "accepted" && (
                        <button onClick={() => setShot((x) => ({ ...x, [slotKey]: { ...s, status: "accepted" } }))}
                          className="rounded-lg border border-[#4ade80]/50 bg-[#4ade80]/10 px-4 py-2 text-base font-bold text-[#86efac]">Accept</button>
                      )}
                    ...
                    {s && !busy[slotKey] && (
                      <div className="mt-3">
                        ...
                        <div className="mt-2 w-full max-w-md">
                          ...
                          <p className="mt-1 text-[14px] text-ink-dim/70">Edits this render, keeping everything else. The logo and deal card stay locked.</p>
                        </div>
                      </div>
                    )}
                  </div>
        ...
        <div className="mt-8 rounded-2xl border border-line bg-surface-1 p-5">
          ...
          {accepted === totalSlots && (
            <button onClick={downloadAll} className="mt-3 rounded-lg bg-accent px-5 py-2.5 text-base font-bold text-black">
      ...
      {lightbox && (
        <div onClick={() => setLightbox("")} className="fixed inset-0 z-[60] flex items-center justify-center bg-black/85 p-6" role="dialog">
          ...
          <button onClick={() => setLightbox("")} className="absolute right-5 top-5 rounded-lg bg-white/10 px-3 py-1.5 text-base font-bold text-white hover:bg-white/20">Close ✕</button>
          <a href={lightbox} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className="absolute bottom-5 rounded-lg bg-white/10 px-3 py-1.5 text-base font-bold text-white hover:bg-white/20">Open in new tab ↗</a>
        </div>
      )}
    </div>
  );

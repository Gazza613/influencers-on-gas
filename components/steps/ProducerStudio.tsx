...
export default function ProducerStudio({ influencerId, name, initialProduction, initialVoiceId = "", initialVoiceName = "", creatives = [], arollRef = "", brollRef = "", voiceModel: initialVoiceModel = "v2", mode = "all", brains = [], initialClientId = "" }: { influencerId: string; name: string; initialProduction: Production; initialVoiceId?: string; initialVoiceName?: string; creatives?: CreativeGuide[]; arollRef?: string; brollRef?: string; voiceModel?: "v2" | "v3"; mode?: "all" | "foundation" | "studio"; brains?: { id: string; name: string }[]; initialClientId?: string }) {
  ...
  function openEdit(i: number, s: Scene) {
    ...
    setEd({ location: s.location || "", blocking: s.blocking || "", shot: s.shot || "", performance: s.performance || "", motion: s.motion_prompt || "", vo: s.vo_line || "", caption: s.caption || "", voAudio: s.vo_audio_url || "", phone: s.phone_screen_url || "", hero: s.hero || "false", ref: s.ref_url || "", captionPos: s.caption_pos || "", captionOff: s.caption_off === "true", extras: s.crowd_extras === true });
  }
              ...
              return (
                <div key={i} id={`prod-scene-${i}`} className="flex gap-4 rounded-xl border border-line bg-surface-1 p-4">
                    ...
                    <div className="w-32 shrink-0">
                      {shot?.reshooting ? (
                        ...
                        <div className="relative">
                          ...
                          {clip.draft && <span className="absolute right-1 bottom-1 rounded bg-[#f59e0b]/90 px-1 py-0.5 text-[10px] font-bold text-black" title="Draft preview: talking shots at 720p, scene shots are a fast ~5s proxy. The voiceover still plays its FULL length here (the video holds its last frame while the voice finishes). Render Final Quality at the Stitch step and each scene shot becomes the full length of its voiceover (Kling, up to 15s) - so the final is never capped at 5s.">720p DRAFT</span>}
                          ...
                          <button onClick={() => setVzoom(clip)} title="Play full size (with her voiceover)" aria-label="Play full size" className="absolute right-1 top-1 z-10 flex h-6 w-6 items-center justify-center rounded-full bg-black/65 text-[11px] text-white transition hover:bg-black/90">👁</button>
                          ...
                          {animatingScenes.has(i) && (
                            <div className="pointer-events-none absolute inset-0 z-20 flex flex-col items-center justify-center gap-1 rounded-lg bg-black/65 text-[10px] font-semibold text-white"><span className="h-5 w-5 animate-spin rounded-full border-2 border-[#60a5fa]/40 border-t-[#60a5fa]" />re-animating…<RenderTimer start={renderStarts[i] ?? Date.now()} hint={String((s as { live_bg?: string }).live_bg) === "true" ? "10-25 min" : s.role === "a-roll" ? "2-6 min" : String((s as { hero?: string }).hero) === "true" ? "10-25 min" : "1-3 min"} /></div>
                          )}
                        </div>
                      ) : !clip?.url && (animatingScenes.has(i) || (wholeBoardBusy && (renderingRole === "" || renderingRole === s.role))) ? (
                        <div className="flex aspect-[9/16] w-full flex-col items-center justify-center gap-1 rounded-lg border border-line bg-surface-2 text-center text-[10px] text-ink-faint"><span className="h-5 w-5 animate-spin rounded-full border-2 border-[#60a5fa]/40 border-t-[#60a5fa]" />rendering…<RenderTimer start={renderStarts[i] ?? Date.now()} hint={String((s as { live_bg?: string }).live_bg) === "true" ? "10-25 min" : s.role === "a-roll" ? "2-6 min" : String((s as { hero?: string }).hero) === "true" ? "10-25 min" : "1-3 min"} /></div>
                      ) : shot?.url ? (
                        <div className="relative">
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                        ...
                        <div className="mt-1.5 space-y-1">
                          ...
                          {s.role === "a-roll" && (
                            <button
                              ...
                              title="Live background: the whole world moves behind her instead of a still backdrop - cinematic, but the slower engine (~10-20 min, no fast preview). Best for a hero moment. Off = the fast, reliable talking-head with a still background. Re-shoot + re-animate to apply."
                            ...
                            >{String(s.live_bg) === "true" ? "🎬 Live bg ✓" : "🎬 Live background"}</button>
                          )}
                  ...
                  <div className="mb-2 flex flex-wrap items-center gap-2">
                    ...
                    {s.role === "b-roll" && (
                      <button onClick={() => toggleExtras(i, s)} title="Background strangers ON/OFF for this scene shot. Scene shots are rendered by a full video model, so extras here genuinely move and behave like real people. Turn OFF for a private/closed space (studio, podcast room, office, boardroom, home). Talking shots are always solo: their engine animates the presenter only, so anyone behind her would be a frozen figure. Re-shoot the scene to apply." className={`tabular rounded border px-1.5 py-0.5 text-[10px] font-bold ${s.crowd_extras === true ? "border-[#60a5fa]/40 bg-[#60a5fa]/10 text-[#93c5fd]" : "border-line text-ink-faint hover:text-ink"}`}>{s.crowd_extras === true ? "👥 Extras" : "🚫 No extras"}</button>
                    )}
                    {s.role === "a-roll" && (
                  ...
                  {/* No length cap: a scene shot now runs exactly as long as its measured voiceover (the clip
                      stretches to the VO), so the old "over the 8s cap, tighten it" warning is gone. */}
                      ...
                      {(<div className="space-y-2 border-t border-line pt-3">
                        ...
                        {s.role === "b-roll" && (
                          <div>
                            ...
                            <button onClick={() => setEd((e) => ({ ...e, hero: e.hero === "true" ? "false" : "true" }))} className={`rounded-lg border px-3 py-1.5 text-xs font-semibold ${ed.hero === "true" ? "border-[#60a5fa] bg-[#60a5fa]/15 text-[#93c5fd]" : "border-line text-ink-dim hover:border-line-strong"}`}>{ed.hero === "true" ? "✨ Hero shot · Premium 4K ✓" : "✨ Make this a Hero shot (Premium 4K)"}</button>
                            <p className="mt-1 text-[10px] text-ink-faint">Renders this scene shot on the premium 4K engine (with natural ambient audio) - richer + slower + pricier. Needs a re-shoot.</p>
                          </div>
                        )}
                        {/* BACKGROUND EXTRAS (b-roll only): turn strangers off for a private/closed space so nobody
                            walks through (a podcast studio, office, boardroom, home). */}
                        {s.role === "b-roll" && (
                          <div>
                            <div className="tabular mb-1 text-[10px] uppercase tracking-[0.2em] text-ink-faint">Background extras</div>
                            <button onClick={() => setEd((e) => ({ ...e, extras: !e.extras }))} className={`rounded-lg border px-3 py-1.5 text-xs font-semibold ${ed.extras ? "border-[#a855f7] bg-[#a855f7]/12 text-[#c79bff]" : "border-line text-ink-dim hover:border-line-strong"}`}>{ed.extras ? "👥 Background people ON" : "🚫 No extras — just the cast"}</button>
                            <p className="mt-1 text-[10px] text-ink-faint">Turn OFF for a private/closed space (a podcast or recording studio, office, boardroom, home) so no strangers appear or walk through. Turn ON only for a genuinely busy public place. Re-shoot the scene to apply.</p>
                          </div>
                        )}
              ...
              {unlocked("voice") ? (
                !needsVoice ? (
                  ...
                  <>
                    ...
                    <details open className="mt-3 rounded-lg border border-line bg-surface-2/30">
                      <summary className="cursor-pointer list-none px-3 py-2 text-[11px] font-semibold text-ink-dim marker:content-['']">⚙ Voice model &amp; preview <span className="font-normal text-ink-faint">· Stable (v2) / Expressive (v3), preview, speed, re-flow</span></summary>
                        ...
                        return (
                          <div key={i} className="w-[122px] shrink-0 rounded-xl border border-line bg-surface-1 p-2 transition hover:border-[#a855f7]/30">
                            <div className="relative aspect-[9/16] w-full overflow-hidden rounded-lg bg-black">
                              ...
                              {busy && <SceneBusy start={renderStarts[i] ?? Date.now()} hint={isReshooting ? "~30s" : String((s as { live_bg?: string }).live_bg) === "true" ? "10-25m" : s.role === "a-roll" ? "2-6m" : "1-3m"} />}
                ...
                <>
                  ...
                  {production?.audio_status === "done" ? (
                    <div className="mt-3 space-y-2">
                      {production?.music_url && <div><div className="tabular text-[10px] uppercase tracking-[0.2em] text-ink-faint">Music bed</div><audio src={production.music_url} controls onLoadedMetadata={(e) => { e.currentTarget.volume = 0.4; }} className="mt-1 h-8 w-full max-w-sm" /></div>}
                      {production?.ambient_url && <div><div className="tabular text-[10px] uppercase tracking-[0.2em] text-ink-faint">Ambient tone <span className="normal-case tracking-normal text-ink-faint">· plays much quieter in the final</span></div><audio src={production.ambient_url} controls onLoadedMetadata={(e) => { e.currentTarget.volume = 0.35; }} className="mt-1 h-8 w-full max-w-sm" /></div>}

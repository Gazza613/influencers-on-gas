                      ...
                      {shot?.reshooting ? (
                        ...
                        <div className="relative">
                          ...
                          {animatingScenes.has(i) && (
                            <div className="pointer-events-none absolute inset-0 z-20 flex flex-col items-center justify-center gap-1 rounded-lg bg-black/65 text-[10px] font-semibold text-white"><span className="h-5 w-5 animate-spin rounded-full border-2 border-[#60a5fa]/40 border-t-[#60a5fa]" />re-animating…<RenderTimer start={renderStarts[i] ?? Date.now()} hint={String((s as { live_bg?: string }).live_bg) === "true" ? "10-25 min" : s.role === "a-roll" ? "2-6 min" : String((s as { hero?: string }).hero) === "true" ? "10-25 min" : speedMode ? "3-8 min" : "15-30 min"} /></div>
                          )}
                        </div>
                      ) : !clip?.url && (animatingScenes.has(i) || (wholeBoardBusy && (renderingRole === "" || renderingRole === s.role))) ? (
                        <div className="flex aspect-[9/16] w-full flex-col items-center justify-center gap-1 rounded-lg border border-line bg-surface-2 text-center text-[10px] text-ink-faint"><span className="h-5 w-5 animate-spin rounded-full border-2 border-[#60a5fa]/40 border-t-[#60a5fa]" />rendering…<RenderTimer start={renderStarts[i] ?? Date.now()} hint={String((s as { live_bg?: string }).live_bg) === "true" ? "10-25 min" : s.role === "a-roll" ? "2-6 min" : String((s as { hero?: string }).hero) === "true" ? "10-25 min" : speedMode ? "3-8 min" : "15-30 min"} /></div>
                      ) : shot?.url ? (
                        <div className="relative">
                          {/* eslint-disable-next-line @next/next/no-img-element */}

    ...
    <div className="mt-6">
      <div className="rounded-xl border border-line bg-surface-1 p-6">
        <div className="flex flex-wrap items-end gap-3">
          {!lockClient && (
            <div>
              <label className="tabular block text-[15px] uppercase tracking-[0.2em] text-ink-faint">Brain</label>
              <select value={clientId} onChange={(e) => { setClientId(e.target.value); setAnswer(""); setHits([]); setTip(null); setErr(""); setSaved(""); setBusy(false); }}
                className="mt-1.5 rounded-lg border border-line bg-surface-2 px-3.5 py-2.5 text-[20px] text-ink outline-none focus:border-accent">
                {clients.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
          )}
          <p className="pb-2 text-[17px] text-ink-faint">
            {mode === "claude"
        ...
        {tip && (
          <div className="mt-4 rounded-lg border border-[#a855f7]/30 bg-[#a855f7]/[0.07] px-4 py-3">
            {tip.from
              ? <>
                  <p className="text-[17px] text-ink-dim">Was: <span className="line-through opacity-70">{tip.from}</span></p>
                  <p className="mt-1 text-[17px] text-[#c79bff]">{tip.why}</p>
                </>
              : <p className="text-[17px] text-[#c79bff]">{tip.why}</p>}
          </div>
        )}

        {err && <p className="mt-4 text-[18px] text-alert">{err}</p>}
      </div>
      ...
      {answer && (
        <div className="mt-5 rounded-xl border border-[#a855f7]/35 bg-[#a855f7]/[0.07] p-6">
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            <span className="tabular text-[15px] uppercase tracking-[0.18em] text-[#c79bff]">
          ...
          <p className="whitespace-pre-wrap text-[21px] leading-relaxed text-ink">
          ...
          {asked && <p className="mt-4 text-[16px] text-ink-faint">You asked: {asked}</p>}
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-[#a855f7]/20 pt-4">
            {saved === "added" ? (
              ...
              <span className="text-[17px] text-ink-dim">Already in the brain, nothing added.</span>
            ) : (
      ...
      {hits.length > 0 && (
        <div className="mt-4">
          <button onClick={() => setOpenSources((o) => !o)} className="text-[18px] font-semibold text-ink-dim hover:text-ink">
            ...
            <ul className="mt-3 space-y-2.5">
              {hits.map((h, i) => (
                <li key={i} className="rounded-lg border border-line bg-surface-1 p-4">
                  ...
                  <div className="text-[17px] leading-relaxed text-ink-dim">
                  ...
                  {(h.metadata?.url as string) && (
                    <a href={h.metadata.url as string} target="_blank" rel="noreferrer"
                      className="mt-2 inline-block text-[16px] text-accent hover:underline">Open the source ↗</a>
                  )}
                </li>
              ))}
            </ul>

  ...
  return (
    <div className="rounded-xl border border-line bg-surface-1 p-6">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2.5 rounded-lg border border-line bg-surface-2 p-3">
        <span className="text-[18px] font-semibold text-ink">Add to the library</span>
        <select value={addKind} onChange={(e) => setAddKind(e.target.value)} disabled={!!busy}
          className="rounded-md border border-line bg-surface-1 px-3 py-2 text-[18px] outline-none focus:border-accent">
        ...
        <button onClick={() => fileRef.current?.click()} disabled={!!busy}
          className="inline-flex items-center gap-2 rounded-md bg-accent px-4 py-2 text-[18px] font-semibold text-white hover:opacity-90 disabled:opacity-50">
      ...
      {err && <p className="mt-2 text-[18px] text-[#fca5a5]">{err}</p>}
              ...
              <div key={g.kind} className="rounded-lg border border-line bg-surface-2">
                <button onClick={() => setOpen((m) => ({ ...m, [g.kind]: !isOpen }))}
                  ...
                  <span className="min-w-0">
                    <span className="text-[18px] font-bold text-ink">{g.label}</span>
                    <span className="tabular ml-2.5 text-[16px] text-ink-faint">{g.assets.length}</span>
                    {g.note && <span className="mt-0.5 block text-[16px] text-ink-dim">{g.note}</span>}
                  </span>
                  <span className="shrink-0 text-[18px] text-ink-faint">{isOpen ? "▲" : "▼"}</span>
                </button>
                    ...
                    {g.assets.map((a) => (
                      <div key={a.id} className="group relative overflow-hidden rounded-lg border border-line bg-surface-1 transition hover:border-line-strong">
                        <button onClick={() => setZoom(a)} title={a.name || ""} className="block w-full">
                          ...
                          <span className="block truncate px-2 pb-1.5 text-[13px] text-ink-faint">{a.name || "untitled"}</span>
                        </button>
                        <button onClick={() => remove(a)} aria-label={`Remove ${a.name || "file"}`} title="Remove from the library"
                          className="absolute right-1.5 top-1.5 rounded-md bg-black/60 px-2 py-0.5 text-[15px] font-bold text-white opacity-0 transition hover:bg-[#f87171] group-hover:opacity-100">✕</button>
                      </div>
                    ))}
                  </div>
                )}
      ...
      {zoom && (
        <div onClick={() => setZoom(null)} role="dialog"
          ...
          <button onClick={() => setZoom(null)}
            className="absolute right-5 top-5 rounded-lg bg-white/10 px-3.5 py-2 text-[18px] font-bold text-white hover:bg-white/20">Close ✕</button>
        </div>
      )}
    </div>
  );

      ...
      {c && (
        <div className="mt-6 space-y-6">
                  ...
                  <div className="mt-3 space-y-2">
                    {(p.platforms || []).map((pl, j) => (
                      <div key={j} className="rounded-md border border-line/70 bg-surface-1 p-2.5">
                        ...
                        <div className="mt-1.5 flex flex-wrap gap-1.5">{(pl.selections || []).map((s, k) => <span key={k} className="rounded-full border border-line px-2 py-0.5 text-sm text-ink-dim">{s}</span>)}</div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <Blk title="Strategic recommendation">
            ...
            <ul className="mt-2 list-disc pl-5 text-lg text-ink-dim">{(c.strategy?.why_it_wins || []).map((w, i) => <li key={i}>{w}</li>)}</ul>
          </Blk>
          ...
          <Blk title="31-day rollout">
            <div className="space-y-3">{(c.rollout || []).map((w, i) => (
              <div key={i} className="rounded-lg border border-line bg-surface-2 p-3">
                ...
                <ul className="mt-1 list-disc pl-5 text-base text-ink-dim">{(w.points || []).map((pt, j) => <li key={j}>{pt}</li>)}</ul>
          ...
          <Blk title="Compliance">
            ...
            <ul className="mt-1.5 list-disc pl-5 text-lg text-ink-dim">{(c.compliance?.points || []).map((p, i) => <li key={i}>{p}</li>)}</ul>
          </Blk>
          ...
          <section className="rounded-xl border border-accent/40 bg-surface-1 p-5">
            ...
            <ul className="mt-2 grid list-disc gap-1 pl-5 text-lg text-ink-dim sm:grid-cols-2">{(c.investment?.engine_includes || []).map((x, i) => <li key={i}>{x}</li>)}</ul>
            {(c.investment?.notes || []).map((n, i) => <p key={i} className="mt-2 text-base text-ink-faint">{n}</p>)}
          </section>
